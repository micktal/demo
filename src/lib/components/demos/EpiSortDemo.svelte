...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4" bind:this={container}>
  <div>
    ...
    <div class="flex flex-wrap gap-2">
      {#each left as it}
        <div class="badge cursor-move" draggable ondragstart={(e)=>drag(e,it)}>{it.label}</div>
      {/each}
    </div>
  </div>
  <div>
    <div class="font-medium mb-2">EPI obligatoires</div>
    <div class="min-h-24 rounded-lg border border-dashed border-gray-400/40 p-3" ondragover={allow} ondrop={(e)=>drop('epi',e)}>
  ...
  <div>
    ...
    <div class="min-h-24 rounded-lg border border-dashed border-gray-400/40 p-3" ondragover={allow} ondrop={(e)=>drop('non',e)}>
